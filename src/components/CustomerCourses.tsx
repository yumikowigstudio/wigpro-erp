...
import { Check, ChevronLeft, ChevronRight, History, Loader2, Search, ShoppingCart, Undo2, X, Ban } from 'lucide-react'
import { db } from '@/lib/firebase'
import { COLLECTIONS, convertTimestamps } from '@/lib/firestore'
import { useAuth } from '@/hooks/useAuth'
import { cancelCourseRights, reverseCourseUse } from '@/lib/courses'
...
export function CustomerCourses({ customerId }: { customerId?: string }) {
  ...
  const [dialog, setDialog] = useState<{ kind: 'reverse' | 'cancel'; course: CustomerCourse; event?: CourseEvent } | null>(null)
  ...
  const open = (kind: 'reverse' | 'cancel', course: CustomerCourse, event?: CourseEvent) => {
    setNote(''); setError(''); setSuccess('')
    setDialog({ kind, course, event })
  }
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (dialog.kind === 'reverse' && dialog.event) await reverseCourseUse(dialog.course, dialog.event, actor, note)
      else await cancelCourseRights(dialog.course, actor, note)
  ...
  return <div className="space-y-4">
    ...
    {loading ? <div className="py-8 flex justify-center"><Loader2 className="animate-spin w-5 h-5" /></div> : !filtered.length ? <p className="py-8 text-center text-sm text-gray-500">ยังไม่มีคอร์สในรายการนี้</p> : <div className="divide-y divide-[var(--border-light)]">
      {filtered.slice(currentPage * 15, currentPage * 15 + 15).map(course => <article key={course.id} className="py-4">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          {course.status === 'active' && !isExpired(course) && course.remainingUnits > 0
            ? <Link className={`${actionClass} bg-emerald-50 text-emerald-800`} href={`/pos?customerId=${encodeURIComponent(course.customerId)}&courseId=${encodeURIComponent(course.id)}&redeem=1`}><ShoppingCart className="w-4 h-4" />เปิดใช้ที่ POS</Link>
            : <span className={`${actionClass} cursor-not-allowed opacity-40`}><ShoppingCart className="w-4 h-4" />เปิดใช้ที่ POS</span>}
        ...
        {expanded === course.id && <div className="mt-4 border-l-2 border-[var(--border-light)] pl-3 space-y-3">
          {events.filter(event => event.courseId === course.id).map(event => <div key={event.id} className="text-sm flex gap-3 justify-between"><div className="min-w-0"><p>{labels[event.kind]} · {event.units} ครั้ง · คงเหลือ {event.balance}</p><p className="text-xs text-gray-500">{date(event.createdAt)} · {event.userName} · {branches.find(branch => branch.id === event.branchId)?.name || event.branchId}</p>{event.serviceName && <p className="text-xs">{[event.serviceName, event.staffName].filter(Boolean).join(' · ')}</p>}{event.note && <p className="text-xs whitespace-pre-wrap break-words mt-1">{event.note}</p>}</div>{manager && course.status === 'active' && event.kind === 'use' && !events.some(other => other.reverseOf === event.id) && <button title="คืนสิทธิ์รายการนี้" aria-label="คืนสิทธิ์รายการนี้" className={`${actionClass} self-start`} onClick={() => open('reverse', course, event)}><Undo2 className="w-4 h-4" /></button>}</div>)}
        </div>}
      </article>)}
    </div>}
    ...
    {dialog && <div className="fixed inset-0 z-[100] bg-black/40 flex items-center justify-center p-4"><form role="dialog" aria-modal="true" aria-label="ปรับสิทธิ์คอร์ส" onSubmit={submit} className="w-full max-w-lg max-h-[90dvh] overflow-auto rounded-lg bg-white shadow-xl p-5 space-y-4">
      <div className="flex items-start justify-between gap-3"><h2 className="text-lg font-semibold">{dialog.kind === 'reverse' ? 'คืนสิทธิ์' : 'ยุติสิทธิ์คงเหลือ'}</h2><button type="button" title="ปิด" disabled={busy} onClick={() => setDialog(null)}><X className="w-5 h-5" /></button></div>
      ...
      <label className="block text-sm">เหตุผล (จำเป็น)<textarea required className={inputClass} rows={3} maxLength={1000} value={note} onChange={event => setNote(event.target.value)} /></label>
      {error && <p role="alert" className="text-sm text-red-700">{error}</p>}
